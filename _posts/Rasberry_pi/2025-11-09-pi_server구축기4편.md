...
그 파일을 Pi의 설정디렉토리(`/home/<REDACTED_USER>/.cloudflared/`)에 옮겨두고,  
...
ingress:
  - hostname: tunnel.<REDACTED-DOMAIN>
...
2. **<REDACTED-DOMAIN>** 도메인을 통해 접속 가능  
3. **localhost:3000 (ttyd)** 으로 트래픽을 전달

이후 커맨드 한 줄로 내 도메인을 DNS에 연결했다.

```bash
cloudflared tunnel route dns <REDACTED-TUNNEL-NAME> tunnel.<REDACTED-DOMAIN>
```

> 이 순간, 드디어 내 서버는 “진짜” 인터넷에 주소를 가진 존재가 되었다.  

이제 누구든 `https://tunnel.<REDACTED-DOMAIN>` 로 접속하면  
